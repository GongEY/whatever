...
class Func:
    ...
    def calculate_pi_wq_lst(self):
        ...
        self.pi_wq_lst = [pow(rho, i) * math.exp(-rho) / math.factorial(i) for i in range(self.factorial_limit)]
    ...
    def get_i_not_in_a_wq(self, i, b):
        ...
        exp_v = 1
        for j in range(b, self.factorial_limit):
            result += self.get_pi_wq(j) * exp_v
            exp_v *= c_r
        return result

    def calculate_H(self):
        self.H_lst = np.zeros(self.factorial_limit)
        self.E_A = 0

        for i in range(self.factorial_limit):
            if i < self.item_size:
    ...
    def calculate_pi_b(self):
        ...
        self.pi_b = [None] * self.factorial_limit
        for i in range(self.factorial_limit):
    ...
    def get_pi_b(self,n):
        result = 0
        test = 0
        for i in range(n, self.factorial_limit):
            test += self.get_pi_wq(i)
